...
interStruct intersection_pt(pair<float,float> start,pair<float,float> end,pair<float,float> pt,float dist_look){
	...
	if(lineDist > dist_look){// pt_check(pt,make_pair(start,end)) == false){
		...
		if(Distance(I1,end) > Distance(I2,end)){ret.p = I2;}
		else{ret.p = I1;}
...
bool loopCheck(pair< pair<float,float>,pair<float,float> > list_pts,pair<float,float> prev_pt,pair<float,float> current_pt){
	...
	if(m1 != m2 && m1 != NAN && m2 != NAN){
		...
		float PA = Distance(pt,list_pts.first);
		float AB = Distance(list_pts.first,list_pts.second);
	
		float PC;
		if(Distance(pt,prev_pt) > Distance(pt,current_pt)){
			PC = Distance(pt,prev_pt);
		}
		else{PC = Distance(pt,current_pt);}

		float CD = Distance(prev_pt,current_pt);
...
new_locStruct new_loc(float eta,float dist_look_inter,pair<float,float> pt_old,float bot_orientation,float delta_prev)
{
	...
	float theta_dot = 0;
	float time1;
	delta_prev = Delta_conv(delta_prev);
	...
	if(time1 >= time_del){
		...
		for(int i = 0;i < 10;i++){
			...
			theta_dot = velocity*tan(Delta_Deconv(delta_next)*PI/180)/length;
			bot_orientation = mod(bot_orientation,360);
		...
		theta_dot = velocity*tan(Delta_Deconv(delta)*PI/180)/length;
		bot_orientation = mod(bot_orientation,360);
		if(theta_dot != 0){
	...
	delta_next = Delta_Deconv(delta_next);
...
pathStruct path(pair<pair<float,float>,pair<float,float> > list_pts,float orientation,float delta_prev)
{
	...
	while(true)
	{
		...
		if(val_lookAhead == false){
			interStruct intersect = intersection_pt(list_pts.first,list_pts.second,pt_bot,dist_look);
			...
			dist_look = Distance(pt_path,pt_bot)/2;
		}	
		eta = eta_fn(pt_bot,pt_path,bot_orientation);
		...
		// cout<<endl;
		new_locStruct new_loc_details = new_loc(eta,dist_look,pt_bot,bot_orientation,delta_prev);
		delta = new_loc_details.delta;
		pt_new = new_loc_details.new_location;
		bot_orientation_new = new_loc_details.orientation;

		if(Distance(pt_bot,list_pts.second) < 10){
			val_lookAhead = true;
			val_loopcheck = loopCheck(list_pts,pt_bot,pt_new);
		...
		if(val_return == true || Distance(pt_new,list_pts.second) < 5){
			pathStruct ret;
			ret.path_ret = true;
...
int main(){
	...
	// pair<float,float> pt = make_pair(100.0,100.0);
	// pair<float,float> pt_path = make_pair(195.0,110.0);
	// pair<float,float> pt_old = make_pair(190.0, 100.0);
	// pair<float,float> pt_new = make_pair(191.5383, 99.7244);

	pair< pair<float,float>,pair<float,float> > list_pts = make_pair(make_pair(100,100),make_pair(295,170));	
	clock_t t1,t2;
	t1 = clock();
	pathStruct path_return = path(list_pts,orientation,delta);
	t2 = clock();
